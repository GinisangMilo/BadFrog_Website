...
export default function Taskbar({ account, connectWallet, disconnectWallet, openStart, setOpenStart, openWindow, windows = [], focusWindow }) {
                ...
                {!account ? (
                  ...
                  <>
                    <div className="mx-2 my-1 px-4 py-2 text-xs truncate border-[2px] bg-white font-bold text-[#000080]" style={{ boxShadow: 'inset -1px -1px #fff, inset 1px 1px #0a0a0a, inset -2px -2px #dfdfdf, inset 2px 2px grey' }}>
                      {resolveDisplayName(account)?.name || `${account.slice(0, 6)}...${account.slice(-4)}`}
                    </div>
      ...
      <div className="flex-1 flex px-2 gap-1 items-center overflow-x-hidden ml-2 h-[24px]">
        {windows.map(win => (
          <button
            ...
            className="flex-1 max-w-[150px] px-2 py-1 text-xs truncate border-[2px] font-bold active:pt-1 bg-[#c0c0c0]"
